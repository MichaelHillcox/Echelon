...
class DbL {
	...
	****************************/
    ...
    function setSettings($value, $name, $value_type) {
		...
		$query = "INSERT INTO ech_config(name, value) VALUES(?, ?)";
		$stmt = $this->mysql->prepare($query) or die('Database Error'. $db->mysql->error);
		$stmt->bind_param('s'.$value_type, $name, $value);
		$stmt->execute();
